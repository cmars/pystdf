...
from pystdf.summary_statistics import SummaryStatistics
...
class PartSummarizer(EventSource):
    ...
    def partSummaryReady(self, dataSource):
        pass
    ...
    def getSiteSynthCounts(self):
        for site, info in self.pcSynth.iteritems():
            ...
            yield [0, site, partCnt[0], None, abrtCnt[0], goodCnt[0], None]
    ...
    def onPrr(self, row):
        partCnt, goodCnt, abrtCnt = self.pcSynth.setdefault(
            row[prr.SITE_NUM], ([0], [0], [0])
        )
        partCnt[0] += 1
    ...
    def onPcr(self, row):
        if row[pcr.HEAD_NUM] == 255:
            ...
            self.pcSummary[row[pcr.SITE_NUM]] = [filterNull(value) for value in row]
