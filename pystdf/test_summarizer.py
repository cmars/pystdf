...
class TestSummarizer(EventSource):
    ...
    def onFtr(self, row):
        ...
        if row[ftr.OPT_FLAG] is not None:
            if row[ftr.OPT_FLAG] & 0x01 > 0:
                countList = self.cyclCntMap.setdefault(
                    (row[ftr.TEST_NUM], row[ftr.CYCL_CNT]), [0]
                )
                countList[0] += 1
            if row[ftr.OPT_FLAG] & 0x02 > 0:
                countList = self.relVadrMap.setdefault(
                    (row[ftr.TEST_NUM], row[ftr.REL_VADR]), [0]
                )
                countList[0] += 1
            if (
                ftr.RTN_STAT < len(row)
                and ftr.RTN_INDX < len(row)
                and row[ftr.RTN_STAT]
                and row[ftr.RTN_INDX]
            ):
                for i, rtnStat in enumerate(row[ftr.RTN_STAT]):
                    if rtnStat > 4 and i < len(
                        row[ftr.RTN_INDX]
                    ):  # A failing return state...
                        pmrIndx = row[ftr.RTN_INDX][i]
                        countList = self.failPinMap.setdefault(
                            (row[ftr.TEST_NUM], pmrIndx), [0]
                        )
                        countList[0] += 1
    ...
    def onTsr(self, row):
        if row[tsr.HEAD_NUM] == 255:
            self.overallTsrs[row[tsr.TEST_NUM]] = [filterNull(value) for value in row]
        else:
