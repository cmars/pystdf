...
from pystdf.table_template import TableTemplate
...
class RecordHeader:
    ...
    def __repr__(self):
        return "<STDF Header, REC_TYP=%d REC_SUB=%d REC_LEN=%d>" % (
            self.typ,
            self.sub,
            self.len,
        )
...
class EndOfRecordException(Exception):
    pass


class InitialSequenceException(Exception):
    pass
...
class StdfRecordMeta(type):
    ...
    def __init__(cls, name, bases, dct):
        ...
        setattr(
            cls, "fieldStdfTypes", [field_type for field_name, field_type in fieldMap]
        )
